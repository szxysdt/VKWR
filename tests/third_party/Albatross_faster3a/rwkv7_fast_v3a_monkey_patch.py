...
HEAD_SIZE = 64
DTYPE = torch.float16
MODEL_PATH = "/dev/shm/rwkv7-g1f-1.5b-20260419-ctx8192.pth"
THIS_DIR = Path(__file__).resolve().parents[3]
CUDA_DIR = THIS_DIR / "third_party/Albatross/faster3a_2605/cuda"

########################################################################################################
# Golden Data Capture Module
########################################################################################################
CAPTURE_MODE = os.environ.get("VKWR_CAPTURE_GOLDEN", "0") == "1"
CAPTURE_LAYER = int(os.environ.get("VKWR_CAPTURE_LAYER", "-1"))
CAPTURE_DIR = str(THIS_DIR / "tests" / "third_party" / "Albatross_faster3a" / "ops" / "monkey_patch")


def _should_capture(layer_id: int) -> bool:
    return CAPTURE_MODE and (CAPTURE_LAYER == -1 or CAPTURE_LAYER == layer_id)


def _save_data(op_name: str, layer_id: int, data_dict: dict) -> None:
    if not _should_capture(layer_id):
        return
    save_dir = os.path.join(CAPTURE_DIR, op_name)
    os.makedirs(save_dir, exist_ok=True)
    existing = [f for f in os.listdir(save_dir) if f.startswith(f"layer_{layer_id}_call_")]
    N = len(existing)
    fname = f"layer_{layer_id}_call_{N}.pt"
    fpath = os.path.join(save_dir, fname)
    data_dict["meta"]["layer_id"] = layer_id
    data_dict["meta"]["call_index"] = N
    torch.save(data_dict, fpath)
    log(f"[CAPTURE] {op_name} layer={layer_id} call={N} -> {fpath}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--cases", default="1x1,1x2,1x4,1x8,1x16,1x32,1x64,1x128,1x256,2x1,4x1,8x1,16x1,32x1,64x1,128x1,256x1,2x2,4x4,8x8,16x16"
    )  # try 1x1024 1024x1 32x32 for extreme tps
    ...
    parser.add_argument(
        "--lowrank-weight", choices=("orig", "transpose", "both"), default="both"
    )  # orig saves VRAM but slows tiny B*T; transpose saves VRAM but slows large B*T
    ...
    log(
        f"start model={MODEL_PATH} wkv={WKV_MODE} emb={EMB_DEVICE} batched_rkv={RKV_MODE} cmix_sparse={CMIX_SPARSE} lowrank_weight={LOWRANK_WEIGHT} orig_linear_groups={groups} pp={pp}"
    )
    log(
        f"fixed fast path: ln=v3a linear=v3a/splitk lowrank={LOWRANK_IN_ROWS_T}/{LOWRANK_OUT_ROWS_T} nofc_rows=by_C row20_t=by_C nofc_t512_rows>={CMIX_NOFC_T512_MIN_ROWS}"
    )
...
class RWKV7:
    def __init__(self) -> None:
        ...
        if emb_cpu is None:
            with torch.cuda.device(emb_dev):
                z["emb.weight"] = torch.ops.rwkv7_v3a_ops.emb_ln0_bf16_to_f16(emb_src.to(device=emb_dev).contiguous(), ln0_w_bf16, ln0_b_bf16)
        else:
    ...
    def forward_from_x(self, x: torch.Tensor, state: list[torch.Tensor], path: PathConfig, all_logits: bool = False, last_indices=None) -> torch.Tensor:
        ...
        for layer in range(L):
            ...
            if T == 1 and path.cmix_mode not in (CMIX_B1T1_SPARSE, CMIX_ROWS2_SPARSE):
                if CAPTURE_MODE:
                    x_before_for_fused = x.detach().clone()
                    shift_state_cmix_before = state[0][layer][1].detach().clone()
                ...
                if CAPTURE_MODE:
                    _save_data(
                        ...
                        layer,
                        {
                            ...
                            "T": T,
                            "C": C,
                            "x": x_before_for_fused.cpu(),
                            "residual": xx.detach().clone().cpu(),
                            "shift_state_in": shift_state_cmix_before.cpu(),
                            "shift_state_after": state[0][layer][1].detach().clone().cpu(),
                            "weight": z[p + "ln2.weight"].detach().clone().cpu(),
                            "bias": z[p + "ln2.bias"].detach().clone().cpu(),
                            "x_k": z[p + "ffn.x_k"].detach().clone().cpu(),
                            "x_out": x.detach().clone().cpu(),
                            "mixed": mixed.detach().clone().cpu(),
                            "meta": {"op_name": "add_layer_norm_cmix_mix_f16"},
                        },
                    )
                xx = self.cmix_from_mixed(layer, mixed, p + "ffn.", path)
            else:
                x, xx = self.add_ln(x, xx, z[p + "ln2.weight"], z[p + "ln2.bias"])
                xx = self.cmix(layer, xx, state[0][layer], p + "ffn.", path)
            if layer + 1 < L:
                p_next = f"blocks.{layer + 1}."
                if LN1_TMIX_FUSE and B == 1 and T == 1:
                    if CAPTURE_MODE:
                        x_before_for_tmix = x.detach().clone()
                        shift_state_tmix_before = state[0][layer + 1][0].detach().clone()
                    ...
                    if CAPTURE_MODE:
                        ...
                        _save_data(
                            ...
                            actual_layer,
                            {
                                ...
                                "T": T,
                                "C": C,
                                "x": x_before_for_tmix.cpu(),
                                "residual": xx.detach().clone().cpu(),
                                "shift_state_in": shift_state_tmix_before.cpu(),
                                "shift_state_after": state[0][layer + 1][0].detach().clone().cpu(),
                                "weight": z[p_next + "ln1.weight"].detach().clone().cpu(),
                                "bias": z[p_next + "ln1.bias"].detach().clone().cpu(),
                                "x_r": z[p_next + "att.x_r"].detach().clone().cpu(),
                                "x_w": z[p_next + "att.x_w"].detach().clone().cpu(),
                                "x_k": z[p_next + "att.x_k"].detach().clone().cpu(),
                                "x_v": z[p_next + "att.x_v"].detach().clone().cpu(),
                                "x_a": z[p_next + "att.x_a"].detach().clone().cpu(),
                                "x_g": z[p_next + "att.x_g"].detach().clone().cpu(),
                                "x_out": outs[0].detach().clone().cpu(),
                                "r": outs[1].detach().clone().cpu(),
                                "w": outs[2].detach().clone().cpu(),
                                "k": outs[3].detach().clone().cpu(),
                                "v": outs[4].detach().clone().cpu(),
                                "a": outs[5].detach().clone().cpu(),
                                "g": outs[6].detach().clone().cpu(),
                                "meta": {"op_name": "add_layer_norm_tmix_mix6_f16"},
                            },
                        )
                ...
                if CAPTURE_MODE:
                    elapsed_t_adv_before = state[2].detach().clone()
                ...
                if CAPTURE_MODE:
                    _save_data(
                        ...
                        999,
                        {
                            "B": B,
                            "amount": T,
                            "elapsed_t_in": elapsed_t_adv_before.cpu(),
                            "elapsed_t_after": state[2].detach().clone().cpu(),
                            "meta": {
                                "op_name": "advance_i32",
                            },
                        },
    ...
    def forward_pp_segment(
        self, x: torch.Tensor, state: list[torch.Tensor], path: PathConfig, start: int, end: int, v_first: torch.Tensor | None
    ...
    def forward_pp_tail(
        self, x: torch.Tensor, state: list[torch.Tensor], T: int, all_logits: bool = False, last_indices=None, advance: bool = True
    ) -> torch.Tensor:
        B = x.size(0)
        if not all_logits:
    ...
    def ln(self, x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.ops.rwkv7_v3a_ops.layer_norm_f16(x.contiguous(), weight, bias)
        if CAPTURE_MODE:
            _save_data(
                "layer_norm_f16",
                999,
                {
                    ...
                    "C": x.shape[2],
                    "x": x_before.cpu(),
                    "weight": weight.detach().clone().cpu(),
                    "bias": bias.detach().clone().cpu(),
                    "out": out.detach().clone().cpu(),
                    "meta": {"op_name": "layer_norm_f16"},
                },
            )
        return out
    ...
    def tmix(
        ...
        if pre_mix is not None:
            ...
            if CAPTURE_MODE:
                shift_state_before = shift_state[0].detach().clone()
            xr, xw, xk, xv, xa, xg = ops.tmix_mix6(
                B, T, C, x.contiguous(), shift_state[0], z[p + "x_r"], z[p + "x_w"], z[p + "x_k"], z[p + "x_v"], z[p + "x_a"], z[p + "x_g"]
            )
            if CAPTURE_MODE:
                _save_data(
                    "tmix_mix6",
                    layer,
                    {
                        ...
                        "shift_state_in": shift_state_before.cpu(),
                        "shift_state_after": shift_state[0].detach().clone().cpu(),
                        "x_r": z[p + "x_r"].detach().clone().cpu(),
                        "x_w": z[p + "x_w"].detach().clone().cpu(),
                        "x_k": z[p + "x_k"].detach().clone().cpu(),
                        "x_v": z[p + "x_v"].detach().clone().cpu(),
                        "x_a": z[p + "x_a"].detach().clone().cpu(),
                        "x_g": z[p + "x_g"].detach().clone().cpu(),
                        "xr": xr.detach().clone().cpu(),
                        "xw": xw.detach().clone().cpu(),
                        "xk": xk.detach().clone().cpu(),
                        "xv": xv.detach().clone().cpu(),
                        "xa": xa.detach().clone().cpu(),
                        "xg": xg.detach().clone().cpu(),
                        "meta": {"op_name": "tmix_mix6"},
                    },
                )
        if pre_mix is not None:
        ...
        if LOWRANK_WEIGHT != "orig" and can_use_lowrank_fused(path.rows) and can_use_lowrank_out_fused(path.rows) and layer != 0:
            if CAPTURE_MODE:
                xw_before = xw.detach().clone()
                xa_before = xa.detach().clone()
                xg_before = xg.detach().clone()
                xv_before = xv.detach().clone()
            w1, a1, g1, v1 = torch.ops.rwkv7_v3a_ops.linear_wagv_rank_in_f16(
                xw.contiguous(), xa.contiguous(), xg.contiguous(), xv.contiguous(), z[p + "w1.t"], z[p + "a1.t"], z[p + "g1.t"], z[p + "v1.t"]
            )
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "xg": xg_before.cpu(),
                        "xv": xv_before.cpu(),
                        "w1_t": z[p + "w1.t"].detach().clone().cpu(),
                        "a1_t": z[p + "a1.t"].detach().clone().cpu(),
                        "g1_t": z[p + "g1.t"].detach().clone().cpu(),
                        "v1_t": z[p + "v1.t"].detach().clone().cpu(),
                        "w1": w1.detach().clone().cpu(),
                        "a1": a1.detach().clone().cpu(),
                        "g1": g1.detach().clone().cpu(),
                        "v1": v1.detach().clone().cpu(),
                        "meta": {"op_name": "linear_wagv_rank_in_f16"},
                    },
                )
            ...
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "a1": a1.detach().clone().cpu(),
                        "g1": g1.detach().clone().cpu(),
                        "meta": {"op_name": "linear_wag_rank_in_f16"},
                    },
                )
        else:
        ...
        if LOWRANK_WEIGHT != "orig" and can_use_lowrank_out_fused(path.rows) and layer != 0 and v1 is not None:
            if CAPTURE_MODE:
                w1_before = w1.detach().clone()
                a1_before = a1.detach().clone()
                g1_before = g1.detach().clone()
                v1_before = v1.detach().clone()
                v_before = v.detach().clone()
                v_first_before = v_first.detach().clone()
            ...
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "g1": g1_before.cpu(),
                        "v1": v1_before.cpu(),
                        "w2_t": z[p + "w2.t"].detach().clone().cpu(),
                        "a2_t": z[p + "a2.t"].detach().clone().cpu(),
                        "g2_t": z[p + "g2.t"].detach().clone().cpu(),
                        "v2_t": z[p + "v2.t"].detach().clone().cpu(),
                        "v": v_before.cpu(),
                        "v_first": v_first_before.cpu(),
                        "v0": z[p + "v0"].detach().clone().cpu(),
                        "w": w.detach().clone().cpu(),
                        "a": a.detach().clone().cpu(),
                        "g": g.detach().clone().cpu(),
                        "v_out": v.detach().clone().cpu(),
                        "meta": {"op_name": "linear_wagv_rank_out_f16"},
                    },
                )
            v_done = True
            ...
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "a": a.detach().clone().cpu(),
                        "g": g.detach().clone().cpu(),
                        "meta": {"op_name": "linear_wag_rank_out_f16"},
                    },
                )
        else:
        ...
        if CAPTURE_MODE:
            _save_data(
                ...
                layer,
                {
                    ...
                    "H": H,
                    "k_in": k_before.cpu(),
                    "k_k": z[p + "k_k"].detach().clone().cpu(),
                    "a0": z[p + "a0"].detach().clone().cpu(),
                    "a12": a.detach().clone().cpu(),
                    "k_a": z[p + "k_a"].detach().clone().cpu(),
                    "new_k": k.detach().clone().cpu(),
                    "neg_kk": neg_kk.detach().clone().cpu(),
                    "kka": kka.detach().clone().cpu(),
                    "meta": {"op_name": "tmix_kk_a_gate"},
                },
            )
            ...
            if LOWRANK_WEIGHT != "orig" and can_use_lowrank_out_fused(path.rows):
                ...
                if CAPTURE_MODE:
                    _save_data(
                        ...
                        {
                            "x": v1_before.cpu(),
                            "weight_t": z[p + "v2.t"].detach().clone().cpu(),
                            "v": v_before.cpu(),
                            "v_first": v_first_before.cpu(),
                            "v0": z[p + "v0"].detach().clone().cpu(),
                            "out": v.detach().clone().cpu(),
                            "meta": {"op_name": "linear_t_vres_f16"},
                        },
                    )
            else:
                v12 = self.linear_rank_out(
                    self.linear_rank_in(xv, z.get(p + "v1"), z.get(p + "v1.t"), path.rows), z.get(p + "v2"), z.get(p + "v2.t"), path.rows
                )
                if CAPTURE_MODE:
                    v_before = v.detach().clone()
                    v12_before = v12.detach().clone()
                ...
                if CAPTURE_MODE:
                    _save_data(
                        ...
                        layer,
                        {
                            ...
                            "C": C,
                            "v": v_before.cpu(),
                            "v_first": v_first.detach().clone().cpu(),
                            "v0": z[p + "v0"].detach().clone().cpu(),
                            "v12": v12_before.cpu(),
                            "out": v.detach().clone().cpu(),
                            "meta": {"op_name": "tmix_vres_gate"},
                        },
                    )
        ...
        if WKV_MODE == "fp32io16":
            ...
            if CAPTURE_MODE:
                wkv_state_before = wkv_state.detach().clone()
                elapsed_t_before = elapsed_t.detach().clone()
            ...
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "C": C,
                        "H": H,
                        "wkv_state_in": wkv_state_before.cpu(),
                        "wkv_state_after": wkv_state.detach().clone().cpu(),
                        "elapsed_t_in": elapsed_t_before.cpu(),
                        "elapsed_t_after": elapsed_t.detach().clone().cpu(),
                        "r": r.detach().clone().cpu(),
                        "w": w.detach().clone().cpu(),
                        "w0": z[p + "w0"].detach().clone().cpu(),
                        "k": k.detach().clone().cpu(),
                        "v": v.detach().clone().cpu(),
                        "a": neg_kk.detach().clone().cpu(),
                        "b": kka.detach().clone().cpu(),
                        "y": y.detach().clone().cpu(),
                        "meta": {
                            "op_name": "wkv_seq_w0_fp16",
                        },
                    },
                )
        else:
            ...
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        "C": C,
                        "x": w_before.cpu(),
                        "vec": z[p + "w0"].detach().clone().cpu(),
                        "out": w_raw.detach().clone().cpu(),
                        "meta": {"op_name": "add_vec"},
                    },
                )
        ...
        y = ops.tmix_lnx_rkvres_xg(
            B, T, C, H, y.contiguous(), r.contiguous(), k.contiguous(), v.contiguous(), z[p + "r_k"], z[p + "ln_x.weight"], z[p + "ln_x.bias"], g.contiguous()
        )
        if CAPTURE_MODE:
            _save_data(
                "tmix_lnx_rkvres_xg",
                layer,
                {
                    ...
                    "k": k.detach().clone().cpu(),
                    "v": v.detach().clone().cpu(),
                    "r_k": z[p + "r_k"].detach().clone().cpu(),
                    "weight": z[p + "ln_x.weight"].detach().clone().cpu(),
                    "bias": z[p + "ln_x.bias"].detach().clone().cpu(),
                    "g": g.detach().clone().cpu(),
                    "out": y.detach().clone().cpu(),
                    "meta": {"op_name": "tmix_lnx_rkvres_xg"},
                },
            )
        return self.linear_orig_layout(y, z[p + "output.weight"], path, "att_c2c"), v_first

    def cmix(self, layer: int, x: torch.Tensor, shift_state: torch.Tensor, p: str, path: PathConfig) -> torch.Tensor:
        z = self.z
        ops = torch.ops.rwkv7_fast_ops_fp16
        B, T, _ = x.shape
        ...
        if path.cmix_mode == CMIX_B1T1_SPARSE:
            return ops.cmix_sparse_one(
                C, z[p + "key.weight.fc"].size(0), x.contiguous(), shift_state[1], z[p + "x_k"], z[p + "key.weight.fc"], z[p + "value.weight"]
            )
        if path.cmix_mode == CMIX_ROWS2_SPARSE:
            return ops.cmix_sparse_rows(
                B, T, C, z[p + "key.weight.fc"].size(0), x.contiguous(), shift_state[1], z[p + "x_k"], z[p + "key.weight.fc"], z[p + "value.weight"]
            )

        if CAPTURE_MODE:
            shift_state_cmix_before = shift_state[1].detach().clone()
        ...
        if CAPTURE_MODE:
            _save_data(
                ...
                layer,
                {
                    ...
                    "x": x.detach().clone().cpu(),
                    "shift_state_in": shift_state_cmix_before.cpu(),
                    "shift_state_after": shift_state[1].detach().clone().cpu(),
                    "x_k": z[p + "x_k"].detach().clone().cpu(),
                    "mixed": mixed.detach().clone().cpu(),
                    "meta": {"op_name": "cmix_mix"},
                },
            )

        return self.cmix_from_mixed(layer, mixed, p, path)

    def cmix_from_mixed(self, layer: int, mixed: torch.Tensor, p: str, path: PathConfig) -> torch.Tensor:
        z = self.z
        ops = torch.ops.rwkv7_fast_ops_fp16
        B, T, _ = mixed.shape
        ...
        if path.cmix_mode == CMIX_B1T1_NOFC:
            if CAPTURE_MODE:
                hid_before = hid.view(-1).detach().clone()
            out = ops.cmix_sparse_down_relu_one(C, z[p + "value.weight"].size(0), hid.view(-1).contiguous(), z[p + "value.weight"])
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "preact": hid_before.cpu(),
                        "value_fc": z[p + "value.weight"].detach().clone().cpu(),
                        "out": out.detach().clone().cpu(),
                        "meta": {"op_name": "cmix_sparse_down_relu_one"},
                    },
                )
            return out
        if path.cmix_mode == CMIX_ROWS2_NOFC:
            ...
            if path.rows >= CMIX_NOFC_T512_MIN_ROWS and C % 512 == 0 and F % 512 == 0:
                if CAPTURE_MODE:
                    hid_before = hid.detach().clone()
                out = ops.cmix_sparse_down_relu_rows_t512(B, T, C, F, hid.contiguous(), z[p + "value.weight"])
                if CAPTURE_MODE:
                    _save_data(
                        ...
                        layer,
                        {
                            ...
                            "value_fc": z[p + "value.weight"].detach().clone().cpu(),
                            "out": out.detach().clone().cpu(),
                            "meta": {"op_name": "cmix_sparse_down_relu_rows_t512"},
                        },
                    )
                return out
            if CAPTURE_MODE:
                hid_before = hid.detach().clone()
            out = ops.cmix_sparse_down_relu_rows(B, T, C, F, hid.contiguous(), z[p + "value.weight"])
            if CAPTURE_MODE:
                _save_data(
                    ...
                    layer,
                    {
                        ...
                        "value_fc": z[p + "value.weight"].detach().clone().cpu(),
                        "out": out.detach().clone().cpu(),
                        "meta": {"op_name": "cmix_sparse_down_relu_rows"},
                    },
                )
            return out
        ...
        if CAPTURE_MODE:
            _save_data(
                ...
                layer,
                {
                    ...
                    "C": C,
                    "x": hid_before.cpu(),
                    "out": k.detach().clone().cpu(),
                    "meta": {"op_name": "relu_square"},
                },
            )
    ...
    def linear(self, x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        if x.numel() == x.size(-1) and weight.size(1) % 64 == 0:
            if CAPTURE_MODE:
                x_before = x.detach().clone()
            out = torch.ops.rwkv7_v3a_ops.linear_f16_m1_splitk(x.contiguous(), weight)
            if CAPTURE_MODE:
                _save_data(
                    ...
                    999,
                    {
                        ...
                        "weight": weight.detach().clone().cpu(),
                        "out": out.detach().clone().cpu(),
                        "meta": {"op_name": "linear_f16_m1_splitk"},
                    },
                )
            return out
        if CAPTURE_MODE:
            x_before = x.detach().clone()
        out = torch.ops.rwkv7_v3a_ops.linear_f16(x.contiguous(), weight)
        if CAPTURE_MODE:
            _save_data(
                "linear_f16",
                999,
    ...
    def linear_orig_layout(self, x: torch.Tensor, weight: torch.Tensor, path: PathConfig, group: str) -> torch.Tensor:
        ...
        def _lc(op: str, params: tuple | None, out: torch.Tensor) -> torch.Tensor:
            if _lx is None:
                return out
            _save_data(
                op,
                ...
                {
                    "x": _lx.cpu(),
                    "weight_orig": weight.detach().clone().cpu(),
                    "params": params,
                    "out": out.detach().clone().cpu(),
                    "meta": {"op_name": op, "group": group, "rows": path.rows},
                },
            )
            return out
        ...
        if path.rows == 1:
            if group == "ffn_key":
                if C == 2560:
                    return _lc(
                        "linear_orig_rows_exact_f16", (128, 2, True), torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 128, 2, True)
                    )
                return _lc(
                    "linear_orig_rows_exact_f16",
                    (128, 2, C <= 1024),
                    torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 128, 2, C <= 1024),
                )
            return _lc(
                "linear_orig_rows_exact_f16",
                (128, 2, group != "att_c2c" or C < 2048),
                torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 128, 2, group != "att_c2c" or C < 2048),
            )
        if path.rows == 2:
            if group == "att_c2c":
                return _lc("linear_orig_rows_exact_f16", (64, 2, True), torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 64, 2, True))
            if group == "ffn_key":
                if C == 2560:
                    return _lc(
                        "linear_orig_rows_exact_f16", (128, 2, False), torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 128, 2, False)
                    )
                if C < 4096:
                    return _lc(
                        "linear_orig_rows_exact_f16", (64, 2, True), torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 64, 2, True)
                    )
                return _lc(
                    "linear_orig_rows_exact_f16", (128, 2, False), torch.ops.rwkv7_v3a_ops.linear_orig_rows_exact_f16(x.contiguous(), weight, 128, 2, False)
                )
        ...
        if path.rows == 3:
            if group == "head":
                if C <= 2048:
                    return _lc("linear_f16_orig", None, torch.ops.rwkv7_v3a_ops.linear_f16_orig(x.contiguous(), weight))
                if C == 2560:
                    return _lc("linear_f16_orig", None, torch.ops.rwkv7_v3a_ops.linear_f16_orig(x.contiguous(), weight))
                return _lc("linear_orig_rows_f16", (3, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 3, 2))
            if group == "ffn_key":
                if C <= 1024:
                    return _lc("linear_orig_rows_cfg_f16", (64, 3, 4), torch.ops.rwkv7_v3a_ops.linear_orig_rows_cfg_f16(x.contiguous(), weight, 64, 3, 4))
                if C == 2048:
                    return _lc("linear_f16_orig", None, torch.ops.rwkv7_v3a_ops.linear_f16_orig(x.contiguous(), weight))
                if C == 2560:
                    return _lc("linear_f16_orig", None, torch.ops.rwkv7_v3a_ops.linear_f16_orig(x.contiguous(), weight))
                return _lc("linear_f16_orig_lt_cfg", (0, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 0))
            if group == "att_c2c":
                if C == 768:
                    return _lc("linear_orig_rows_f16", (1, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 1, 2))
                if C == 1024:
                    return _lc("linear_orig_rows_f16", (2, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 2, 2))
                if C == 2048:
                    return _lc("linear_orig_rows_f16", (3, 4), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 3, 4))
                if C == 2560:
                    return _lc("linear_orig_rows_f16", (3, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 3, 2))
                return _lc("linear_f16_orig_lt_cfg", (0, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 2))
            return _lc("linear_orig_rows_cfg_f16", (64, 3, 4), torch.ops.rwkv7_v3a_ops.linear_orig_rows_cfg_f16(x.contiguous(), weight, 64, 3, 4))
        if path.rows == 4:
            if group == "ffn_key":
                if C <= 1024:
                    return _lc("linear_orig_rows_cfg_f16", (64, 2, 4), torch.ops.rwkv7_v3a_ops.linear_orig_rows_cfg_f16(x.contiguous(), weight, 64, 2, 4))
                if C == 2048:
                    return _lc("linear_f16_orig", None, torch.ops.rwkv7_v3a_ops.linear_f16_orig(x.contiguous(), weight))
                if C == 2560:
                    return _lc("linear_f16_orig", None, torch.ops.rwkv7_v3a_ops.linear_f16_orig(x.contiguous(), weight))
            ...
            if group == "att_c2c":
                if C <= 1024:
                    return _lc("linear_orig_rows_f16", (2, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 2, 2))
                if C == 2048:
                    return _lc("linear_orig_rows_f16", (4, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 4, 2))
                if C == 2560:
                    return _lc("linear_orig_rows_f16", (4, 2), torch.ops.rwkv7_v3a_ops.linear_orig_rows_f16(x.contiguous(), weight, 4, 2))
                return _lc("linear_f16_orig_lt_cfg", (0, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 2))
        if group == "head":
            if C == 768:
                if 192 <= path.rows < 256:
                    return _lc("linear_f16_orig_lt_cfg", (128, 3), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 3))
                if 96 <= path.rows < 160:
                    return _lc("linear_f16_orig_lt_cfg", (0, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 1))
            if C == 1024:
                ...
                if 192 <= path.rows < 256:
                    return _lc("linear_f16_orig_lt_cfg", (0, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 2))
                if 96 <= path.rows < 160:
                    return _lc("linear_f16_orig_lt_cfg", (32, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 1))
            if C == 2048:
                if 256 <= path.rows < 384:
                    return _lc("linear_f16_orig_lt_cfg", (32, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 0))
                if 192 <= path.rows < 256:
                    return _lc("linear_f16_orig_lt_cfg", (32, 6), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 6))
            ...
            if C == 2560:
                if path.rows >= 256:
                    return _lc("linear_f16_orig_lt_cfg", (32, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 0))
                if path.rows >= 192:
                    return _lc("linear_f16_orig_lt_cfg", (0, 5), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 5))
                if path.rows >= 160:
                    return _lc("linear_f16_orig_lt_cfg", (32, 5), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 5))
                if path.rows >= 128:
                    return _lc("linear_f16_orig_lt_cfg", (0, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 1))
                if path.rows >= 96:
                    return _lc("linear_f16_orig_lt_cfg", (32, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 0))
                ...
                if path.rows >= 72:
                    return _lc("linear_f16_orig_lt_cfg", (32, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 1))
            if path.rows >= 1024:
                return _lc("linear_f16_orig_lt_cfg", (128, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 0))
            if path.rows >= 512:
                return _lc("linear_f16_orig_lt_cfg", (0, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 2))
            if path.rows >= 384:
                return _lc("linear_f16_orig_lt_cfg", (128, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 2))
            if path.rows >= 256:
                return _lc("linear_f16_orig_lt_cfg", (0, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 1))
            if path.rows >= 192:
                return _lc("linear_f16_orig_lt_cfg", (128, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 0))
            ...
            if path.rows >= 96:
                return _lc("linear_f16_orig_lt_cfg", (32, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 1))
            if path.rows >= 80:
                return _lc("linear_f16_orig_lt_cfg", (32, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 2))
            if path.rows >= 72:
                return _lc("linear_f16_orig_lt_cfg", (128, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 2))
        if group == "att_c2c":
            ...
            if C == 768:
                if 256 <= path.rows < 384:
                    return _lc("linear_f16_orig_lt_cfg", (128, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 1))
                if 96 <= path.rows < 112:
                    return _lc("linear_f16_orig_lt_cfg", (32, 3), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 3))
            if C == 1024:
                if 256 <= path.rows < 384:
                    return _lc("linear_f16_orig_lt_cfg", (128, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 0))
                if 96 <= path.rows < 112:
            ...
            if C == 2048:
                ...
                if 192 <= path.rows < 256:
                    return _lc("linear_f16_orig_lt_cfg", (128, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 0))
                if 96 <= path.rows < 112:
                    return _lc("linear_f16_orig_lt_cfg", (32, 4), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 4))
            if C == 2560:
                if path.rows >= 256:
                    return _lc("linear_f16_orig_lt_cfg", (0, 1), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 1))
                if path.rows >= 160:
            ...
            if path.rows >= 32:
                return _lc("linear_f16_orig_lt_cfg", (0, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 0))
            if path.rows >= 24:
                return _lc("linear_f16_orig_lt_cfg", (0, 6), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 6))
            if path.rows >= 12:
                return _lc("linear_f16_orig_lt_cfg", (0, 0), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 0))
            if path.rows >= 5:
                return _lc("linear_f16_orig_lt_cfg", (0, 2), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 2))
        if group == "ffn_key":
            ...
            if C == 2048 and 128 <= path.rows < 160:
                return _lc("linear_f16_orig_lt_cfg", (0, 3), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 3))
            if C == 2560:
                if path.rows >= 192:
                    return _lc("linear_f16_orig_lt_cfg", (32, 5), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 5))
                if path.rows >= 160:
                    return _lc("linear_f16_orig_lt_cfg", (0, 4), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 4))
                if path.rows >= 128:
                    return _lc("linear_f16_orig_lt_cfg", (32, 5), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 32, 5))
                if path.rows >= 112:
                    return _lc("linear_f16_orig_lt_cfg", (128, 4), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 4))
                if path.rows >= 96:
                    return _lc("linear_f16_orig_lt_cfg", (128, 4), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 128, 4))
                if path.rows >= 80:
                    return _lc("linear_f16_orig_lt_cfg", (0, 3), torch.ops.rwkv7_v3a_ops.linear_f16_orig_lt_cfg(x.contiguous(), weight, 0, 3))
    ...
    def linear_rank_in(self, x: torch.Tensor, weight: torch.Tensor, weight_t: torch.Tensor, rows: int) -> torch.Tensor:
        if weight_t is not None and rows <= LOWRANK_IN_ROWS_T:
            if CAPTURE_MODE:
                x_before = x.detach().clone()
            out = torch.ops.rwkv7_v3a_ops.linear_t_f16(x.contiguous(), weight_t)
            if CAPTURE_MODE:
                _save_data(
                    "linear_t_f16",
                    999,
                    {
                        "x": x_before.cpu(),
                        "weight_t": weight_t.detach().clone().cpu(),
                        "out": out.detach().clone().cpu(),
                        "meta": {"op_name": "linear_t_f16"},
                    },
                )
            return out
    ...
    def linear_rank_out_act(self, x: torch.Tensor, weight: torch.Tensor, weight_t: torch.Tensor, rows: int, act: int) -> torch.Tensor:
        if weight_t is not None and C >= LOWRANK_FUSED_MIN_C and rows <= LOWRANK_OUT_ROWS_T:
            if CAPTURE_MODE:
                x_before = x.detach().clone()
            out = torch.ops.rwkv7_v3a_ops.linear_t_act_f16(x.contiguous(), weight_t, act)
            if CAPTURE_MODE:
                _save_data(
                    "linear_t_act_f16",
                    999,
    ...
    def add(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        if CAPTURE_MODE:
            x_before = x.detach().clone()
            y_before = y.detach().clone()
        out = torch.ops.rwkv7_v3a_ops.add_f16(x.contiguous(), y.contiguous())
        if CAPTURE_MODE:
            _save_data(
                "add_f16",
                999,
    ...
    def add_ln(self, x: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if CAPTURE_MODE:
            x_before = x.detach().clone()
            res_before = residual.detach().clone()
        ...
        if CAPTURE_MODE:
            _save_data(
                ...
                999,
                {
                    ...
                    "C": x.shape[2],
                    "x": x_before.cpu(),
                    "residual": res_before.cpu(),
                    "weight": weight.detach().clone().cpu(),
                    "bias": bias.detach().clone().cpu(),
                    "x_out": outs[0].detach().clone().cpu(),
                    "normed": outs[1].detach().clone().cpu(),
                    "meta": {"op_name": "add_layer_norm_f16"},
                },
            )
    ...
    def add_last_ln(self, x: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
        if CAPTURE_MODE:
            x_before = x.detach().clone()
            res_before = residual.detach().clone()
        out = torch.ops.rwkv7_v3a_ops.add_last_layer_norm_f16(x.contiguous(), residual.contiguous(), weight, bias)
        if CAPTURE_MODE:
            _save_data(
                ...
                999,
                {
                    ...
                    "bias": bias.detach().clone().cpu(),
                    "out": out.detach().clone().cpu(),
                    "meta": {"op_name": "add_last_layer_norm_f16"},
                },
            )
        return out
...
def bench_case(model: RWKV7, B: int, T: int, warmup: int, iters: int, profile_range: bool) -> None:
    ...
    # DEBUG: disabled CUDA graph to allow print statements
    # if pp_enabled():
    #     segments = pp_segments()
    #     stage_inputs = []
    #     stage_vfirst = []
