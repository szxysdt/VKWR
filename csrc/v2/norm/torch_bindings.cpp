...
std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_fn(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, torch::Tensor slot_indices, double eps) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(residual, "residual");
  ...
  TORCH_CHECK(slot_indices.dim() == 1, "slot_indices must be 1D");
  ...
  return add_layer_norm_cmix_mix_f16_cuda(x, residual, shift_state, weight, bias, x_k, slot_indices, eps);
}

std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_cfg_fn(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, torch::Tensor slot_indices, double eps, int64_t threads) {
  ...
  return add_layer_norm_cmix_mix_f16_cfg_cuda(x, residual, shift_state, weight, bias, x_k, slot_indices, eps, static_cast<int>(threads));
}

std::vector<torch::Tensor> add_layer_norm_cmix_mix_f16_scalar_stats_fn(torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias, torch::Tensor x_k, torch::Tensor slot_indices, double eps) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(residual, "residual");
  check_half_cuda_contig(shift_state, "shift_state");
  check_half_cuda_contig(weight, "weight");
  ...
  TORCH_CHECK(bias.dim() == 1 && bias.size(0) == 4096, "bias shape mismatch");
  TORCH_CHECK(x_k.dim() == 1 && x_k.size(0) == 4096, "x_k shape mismatch");
  return add_layer_norm_cmix_mix_f16_scalar_stats_cuda(x, residual, shift_state, weight, bias, x_k, slot_indices, eps);
}
...
std::vector<torch::Tensor> add_layer_norm_tmix_mix6_f16_fn(
  ...
  return add_layer_norm_tmix_mix6_f16_cuda(
      x, residual, shift_state, weight, bias, x_r, x_w, x_k, x_v, x_a, x_g, slot_indices, eps);
}

std::vector<torch::Tensor> add_layer_norm_tmix_mix6_f16_cfg_fn(
    torch::Tensor x, torch::Tensor residual, torch::Tensor shift_state, torch::Tensor weight, torch::Tensor bias,
    torch::Tensor x_r, torch::Tensor x_w, torch::Tensor x_k, torch::Tensor x_v, torch::Tensor x_a, torch::Tensor x_g,
    torch::Tensor slot_indices, double eps, int64_t threads) {
  TORCH_CHECK(threads == 256 || threads == 512 || threads == 1024, "threads must be 256, 512, or 1024");
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(residual, "residual");
  check_half_cuda_contig(shift_state, "shift_state");
  ...
  return add_layer_norm_tmix_mix6_f16_cfg_cuda(
      x, residual, shift_state, weight, bias, x_r, x_w, x_k, x_v, x_a, x_g, slot_indices, eps, static_cast<int>(threads));
}
...
TORCH_LIBRARY(vkwr_v2_norm, m) {
  ...
  m.def("add_layer_norm_cmix_mix_f16(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_k, Tensor slot_indices, float eps=1e-5) -> Tensor[]");
  m.impl("add_layer_norm_cmix_mix_f16", c10::kCUDA, &add_layer_norm_cmix_mix_f16_fn);

  m.def("add_layer_norm_cmix_mix_f16_cfg(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_k, Tensor slot_indices, float eps, int threads) -> Tensor[]");
  m.impl("add_layer_norm_cmix_mix_f16_cfg", c10::kCUDA, &add_layer_norm_cmix_mix_f16_cfg_fn);

  m.def("add_layer_norm_cmix_mix_f16_scalar_stats(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_k, Tensor slot_indices, float eps=1e-5) -> Tensor[]");
  m.impl("add_layer_norm_cmix_mix_f16_scalar_stats", c10::kCUDA, &add_layer_norm_cmix_mix_f16_scalar_stats_fn);

  m.def("add_layer_norm_tmix_mix6_f16(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_r, Tensor x_w, Tensor x_k, Tensor x_v, Tensor x_a, Tensor x_g, Tensor slot_indices, float eps=1e-5) -> Tensor[]");
  m.impl("add_layer_norm_tmix_mix6_f16", c10::kCUDA, &add_layer_norm_tmix_mix6_f16_fn);

  m.def("add_layer_norm_tmix_mix6_f16_cfg(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_r, Tensor x_w, Tensor x_k, Tensor x_v, Tensor x_a, Tensor x_g, Tensor slot_indices, float eps, int threads) -> Tensor[]");
  m.impl("add_layer_norm_tmix_mix6_f16_cfg", c10::kCUDA, &add_layer_norm_tmix_mix6_f16_cfg_fn);

  m.def("add_layer_norm_tmix_mix6_f16_scalar_stats(Tensor x, Tensor residual, Tensor(a!) shift_state, Tensor weight, Tensor bias, Tensor x_r, Tensor x_w, Tensor x_k, Tensor x_v, Tensor x_a, Tensor x_g, Tensor slot_indices, float eps=1e-5) -> Tensor[]");
  m.impl("add_layer_norm_tmix_mix6_f16_scalar_stats", c10::kCUDA, &add_layer_norm_tmix_mix6_f16_scalar_stats_fn);
}

REGISTER_EXTENSION(_v2_norm_C)
