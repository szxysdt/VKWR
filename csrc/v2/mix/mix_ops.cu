...
__global__ void tmix_mix6_kernel(
  ...
  __half2 prev2;
  if (t == 0) {
    prev2 = load_h2(shift_state + static_cast<int64_t>(slot_id) * C + c);
  } else {
  ...
  if (t == T - 1) {
    *reinterpret_cast<__half2*>(shift_state + static_cast<int64_t>(slot_id) * C + c) = cur2;
  }
}
...
#pragma unroll
  ...
  const int b = static_cast<int>(pair_idx >> 11);
  const int slot_id = slot_indices[b];
  const int64_t state_base = static_cast<int64_t>(slot_id) * 4096;
  const __half2 cur2 = load_h2(x + idx);
  ...
  *reinterpret_cast<__half2*>(shift_state + state_base + c) = cur2;
  }
}
...
__global__ void cmix_sparse_up_one_kernel(
  ...
  const int slot_id = slot_indices[0];
  const auto x2 = reinterpret_cast<const __half2*>(x);
  const auto p2 = reinterpret_cast<const __half2*>(shift_state + static_cast<int64_t>(slot_id) * C);
...
__global__ void cmix_sparse_up_rows_kernel(
  ...
  const auto p2 = (t == 0)
      ? reinterpret_cast<const __half2*>(shift_state + static_cast<int64_t>(slot_id) * C)
...
__global__ void cmix_sparse_copy_zero_one_kernel(
  ...
  if (i < n4) {
    const int slot_id = slot_indices[0];
    const int64_t dst = static_cast<int64_t>(slot_id) * (C / 8) + i;
    reinterpret_cast<int4*>(shift_state)[dst] = reinterpret_cast<const int4*>(x)[i];
...
__global__ void cmix_sparse_copy_zero_rows_kernel(
  ...
  if (i < state_vec4) {
    ...
    const int slot_id = slot_indices[b];
    const int c4 = static_cast<int>(i % (C / 8));
    const int64_t dst = static_cast<int64_t>(slot_id) * (C / 8) + c4;
    reinterpret_cast<int4*>(shift_state)[dst] =
...
__global__ void cmix_mix_kernel(
  ...
  const __half2 cur2 = load_h2(x + idx);
  const __half2 prev2 = (t == 0) ? load_h2(shift_state + static_cast<int64_t>(slot_id) * C + c) : load_h2(x + idx - C);
  const float2 cur = __half22float2(cur2);
  const float2 prev = __half22float2(prev2);
...
std::vector<at::Tensor> tmix_mix6_t1_c4096_cuda_impl(
  ...
  if (half_math) {
    tmix_mix6_t1_c4096_kernel<true, Vec><<<blocks, threads, 0, stream>>>(
        x.data_ptr<dtype>(), slot_indices.data_ptr<int>(), shift_state.data_ptr<dtype>(),
...
std::vector<at::Tensor> tmix_mix6_t1_c4096_cuda(
    ...
    bool half_math) {
  if (vec == 2) {
    return tmix_mix6_t1_c4096_cuda_impl<2>(B, x, slot_indices, shift_state, x_r, x_w, x_k, x_v, x_a, x_g, threads, half_math);
  }
  if (vec == 4) {
    return tmix_mix6_t1_c4096_cuda_impl<4>(B, x, slot_indices, shift_state, x_r, x_w, x_k, x_v, x_a, x_g, threads, half_math);
  }
  if (vec == 8) {
    return tmix_mix6_t1_c4096_cuda_impl<8>(B, x, slot_indices, shift_state, x_r, x_w, x_k, x_v, x_a, x_g, threads, half_math);
  }
  return tmix_mix6_t1_c4096_cuda_impl<1>(B, x, slot_indices, shift_state, x_r, x_w, x_k, x_v, x_a, x_g, threads, half_math);
}
...
std::vector<at::Tensor> tmix_kk_a_gate_cuda(
    ...
    bool update_shift) {
  (void)C;
  TORCH_CHECK(C == H * HEAD_SIZE, "C must equal H * HEAD_SIZE");
  ...
  if (update_shift) {
    tmix_kk_a_gate_kernel<true><<<blocks, WARPS_PER_BLOCK * 32, 0, stream>>>(
        ...
        k_a.data_ptr<dtype>(), x.data_ptr<dtype>(), slot_indices.data_ptr<int>(),
        shift_state.data_ptr<dtype>(), new_k.data_ptr<dtype>(), neg_kk.data_ptr<dtype>(),
        kka.data_ptr<dtype>(), bth_size);
  } else {
    tmix_kk_a_gate_kernel<false><<<blocks, WARPS_PER_BLOCK * 32, 0, stream>>>(
        H, k.data_ptr<dtype>(), k_k.data_ptr<dtype>(), a0.data_ptr<dtype>(), a12.data_ptr<dtype>(),
        k_a.data_ptr<dtype>(), nullptr, nullptr, nullptr,
