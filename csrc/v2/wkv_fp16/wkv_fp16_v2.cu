...
__global__ void __launch_bounds__(CLONE_N, 2) wkv_fp16_v1_clone_kernel(
  ...
  const int lane = i % 32;
  const int b_slot = slot_indices ? slot_indices[b] : b;

  __shared__ __align__(256) half2 state_smem[CLONE_N][CLONE_N / 2];

  state_ptr += b_slot * C * CLONE_N + h * CLONE_N * CLONE_N;
...
#pragma unroll
    ...
    half sa = sa2.x + sa2.y;
    sa2 = {sa, sa};
    ((F*)w)[i] = w_delta_maybe_w0<AddW0>(((F*)w)[i], w0_ptr, h * CLONE_N + i, elapsed_t[b_slot] + h * CLONE_N + i + tt);
...
__global__ __launch_bounds__(N, 2) void wkv_fp16_v1_exact_kernel(
  ...
  const int i = threadIdx.x;
  const int lane = i % 32;
  const int b_slot = slot_indices ? slot_indices[b_id] : b_id;

  __shared__ __align__(256) half2 state_smem[N][HALF2_N];
  state_ptr += b_slot * C * N + h * N * N;
...
#pragma unroll
    ...
    half sa = sa2.x + sa2.y;
    sa2 = {sa, sa};
    ((half*)w)[i] = w_delta_maybe_w0<AddW0>(((half*)w)[i], w0_ptr, h * N + i, elapsed_t[b_slot] + h * N + i + tt);
...
__global__ __launch_bounds__(N, 2) void wkv_fp16_seq_v2_kernel(
  ...
  __shared__ __align__(256) half2 state_smem[N][HALF2_N];
  state_ptr += static_cast<int64_t>(b_slot) * C * N + h * N * N;
...
#pragma unroll
    ...
    half sa = sa2.x + sa2.y;
    sa2 = {sa, sa};
    ((half*)w[cur])[i] = w_delta_maybe_w0<AddW0>(((half*)w[cur])[i], w0_ptr, h * N + i, elapsed_t[b_slot] + h * N + i + tt);
    __syncthreads();
...
__global__ __launch_bounds__(N, 1) void wkv_fp16_one_direct_kernel(
  ...
  __shared__ __align__(256) half2 state_smem[N][HALF2_N];
  half* state_base = state_ptr + static_cast<int64_t>(b_slot) * C * N + h * N * N;
...
#pragma unroll
  ...
  half sa = sa2.x + sa2.y;
  sa2 = {sa, sa};
  ((half*)w)[i] = w_delta_maybe_w0<AddW0>(((half*)w)[i], w0_ptr, h * N + i, elapsed_t[b_slot] + h * N + i);
  __syncthreads();
...
void wkv_seq_v2_cuda_impl(
  ...
  assert(C == H * N);
  if (T == 1) {
    wkv_one_v2_cuda_impl(B, C, H, state, r, w, w0_ptr, add_w0, k, v, a, b, y, slot_indices_ptr, elapsed_t);
    return;
  }
...
void wkv_seq_v2_cuda(
    ...
    at::Tensor elapsed_t) {
  const int* slot_indices_ptr = slot_indices.defined() && slot_indices.numel() > 0
      ? slot_indices.data_ptr<int>()
      : nullptr;
  wkv_seq_v2_cuda_impl(B, T, C, H, state, r, w, nullptr, false, k, v, a, b, y, slot_indices_ptr, elapsed_t);
}
...
void wkv_seq_w0_v2_cuda(
  ...
  const int* slot_indices_ptr = slot_indices.defined() && slot_indices.numel() > 0
      ? slot_indices.data_ptr<int>()
      : nullptr;
  wkv_seq_v2_cuda_impl(B, T, C, H, state, r, w, reinterpret_cast<const half*>(w0.data_ptr()), true, k, v, a, b, y, slot_indices_ptr, elapsed_t);
}
...
void wkv_one_v2_cuda(
  ...
  const int* slot_indices_ptr = slot_indices.defined() && slot_indices.numel() > 0
      ? slot_indices.data_ptr<int>()
      : nullptr;
  wkv_one_v2_cuda_impl(B, C, H, state, r, w, nullptr, false, k, v, a, b, y, slot_indices_ptr, elapsed_t);
}
...
void wkv_one_w0_v2_cuda(
  ...
  const int* slot_indices_ptr = slot_indices.defined() && slot_indices.numel() > 0
      ? slot_indices.data_ptr<int>()
      : nullptr;
  wkv_one_v2_cuda_impl(B, C, H, state, r, w, reinterpret_cast<const half*>(w0.data_ptr()), true, k, v, a, b, y, slot_indices_ptr, elapsed_t);
}
