...
std::vector<torch::Tensor> add_layer_norm_tmix_mix6_f16_varlen_fn(
    ...
    torch::Tensor req_id, torch::Tensor slot_indices, double eps) {
  ...
  TORCH_CHECK(req_id.is_cuda(), "req_id must be CUDA tensor (device-side read)");
  ...
  TORCH_CHECK(slot_indices.dim() == 1 && slot_indices.size(0) > 0,
              "slot_indices must be 1D with size > 0");
  ...
  return add_layer_norm_tmix_mix6_f16_cuda_varlen(
      ...
      query_start_loc, req_id, slot_indices, eps);
}
...
TORCH_LIBRARY_FRAGMENT(vkwr_v2_5_norm, m) {
  m.def(
      ...
      "query_start_loc, Tensor req_id, Tensor slot_indices, float eps=1e-5) "
      "-> Tensor[]");
...
REGISTER_EXTENSION(_v2_5_norm_C)
