...
__launch_bounds__(HeadSize, 2) __global__ void wkv_fp32_v2_kernel(
    const int* __restrict__ query_start_loc, const int* __restrict__ slot_indices, int C, int H,
    float* __restrict__ state_ptr, const io_t* __restrict__ r_ptr,
    const io_t* __restrict__ w_ptr, const io_t* __restrict__ k_ptr,
    const io_t* __restrict__ v_ptr, const io_t* __restrict__ a_ptr,
    const io_t* __restrict__ b_ptr, io_t* __restrict__ y_ptr) {
  ...
  float* state_base =
      state_ptr + (static_cast<int64_t>(slot_id) * H * HeadSize * HeadSize +
...
#pragma unroll
  ...
  for (int t = 0; t < my_t; ++t) {
    const int64_t idx = (token_base + t) * C + c_base + i;
    __syncthreads();
...
__launch_bounds__(WARP_THREADS, 4) void wkv_fp32_v2_small_warp_kernel(
  ...
  const int state_base_idx = ((slot_id * H + h) * N + row) * N;
  const int my_t = query_start_loc[b_id + 1] - query_start_loc[b_id];
  const int token_base = query_start_loc[b_id];
  ...
  for (int t = 0; t < my_t; ++t) {
    ...
    for (int j = lane; j < N; j += WARP_THREADS) {
      sa += state_ptr[state_base_idx + j] * load_io(a_ptr, token + j);
    }
    ...
    for (int j = lane; j < N; j += WARP_THREADS) {
      ...
      const float s =
          state_ptr[state_base_idx + j] * w_eff(load_io(w_ptr, idx)) +
          vv * load_io(k_ptr, idx) + sa * load_io(b_ptr, idx);
      state_ptr[state_base_idx + j] = s;
...
static bool use_small_auto(int B, int max_t) {
#ifdef _IO_FP16_
  return (max_t == 1 && B <= 96) || (max_t == 2 && B <= 21) ||
         (max_t == 3 && B <= 3) || (max_t == 4 && (B == 1 || B == 3)) ||
         (B == 1 && max_t >= 5 && max_t <= 11);
#else
  return (max_t == 1) || (max_t == 2 && B <= 96) ||
         (max_t == 3 && (B <= 4 || B == 6)) ||
         (max_t == 4 && (B == 1 || B == 3)) ||
         (B == 1 && max_t >= 5 && max_t <= 9);
#endif
...
void wkv_fp32_v2_cuda_varlen(int B, int max_t, const int* query_start_loc,
                             const int* slot_indices,
                             int C, int H, int mode, at::Tensor state,
  ...
  if (mode == 3) {
    wkv_fp32_v2_short_block_kernel<<<dim3(N, H, B), dim3(BLOCK_THREADS), 0,
                                      stream>>>(
        query_start_loc, slot_indices, C, H, state.data_ptr<float>(),
