...
void check_wkv_fp32_varlen_slot_indices(torch::Tensor slot_indices, int64_t B) {
...
void wkv_forward_fp32_varlen(int64_t B, int64_t total_tokens, int64_t max_t,
  ...
  check_wkv_fp32_varlen_inputs(B, C, H, total_tokens, query_start_loc, state, r,
                                 w, k, v, a, b, y, slot_indices);
  check_wkv_fp32_varlen_slot_indices(slot_indices, B);
  ...
  wkv_fp32_v2_cuda_varlen(static_cast<int>(B), static_cast<int>(max_t), qsl_ptr,
                           slot_ptr, static_cast<int>(C), static_cast<int>(H), 0,
                           state, r, w, k, v, a, b, y);
}
...
void wkv_forward_seq_fp32_varlen(int64_t B, int64_t total_tokens, int64_t max_t,
  ...
  const int* slot_ptr = slot_indices.data_ptr<int>();
  wkv_fp32_v2_cuda_varlen(static_cast<int>(B), static_cast<int>(max_t), qsl_ptr,
                           slot_ptr, static_cast<int>(C), static_cast<int>(H), 1,
                           state, r, w, k, v, a, b, y);
}
...
void wkv_forward_small_fp32_varlen(int64_t B, int64_t total_tokens,
  ...
  const int* slot_ptr = slot_indices.data_ptr<int>();
  wkv_fp32_v2_cuda_varlen(static_cast<int>(B), static_cast<int>(max_t), qsl_ptr,
                           slot_ptr, static_cast<int>(C), static_cast<int>(H), 2,
                           state, r, w, k, v, a, b, y);
}
...
void wkv_forward_block_fp32_varlen(int64_t B, int64_t total_tokens,
  ...
  const int* slot_ptr = slot_indices.data_ptr<int>();
  wkv_fp32_v2_cuda_varlen(static_cast<int>(B), static_cast<int>(max_t), qsl_ptr,
                          slot_ptr, static_cast<int>(C), static_cast<int>(H), 3,
                          state, r, w, k, v, a, b, y);
}
...
TORCH_LIBRARY_FRAGMENT(vkwr_v2_5_wkv, m) {
  ...
  m.def(
      ...
      "int H, Tensor slot_indices, Tensor "
      "query_start_loc, Tensor(a!) state, Tensor r, Tensor w, Tensor k, Tensor "
      "v, Tensor a, Tensor b, Tensor(a!) y) -> ()");
  ...
  m.def(
      ...
      "C, int H, Tensor slot_indices, Tensor "
      "query_start_loc, Tensor(a!) state, Tensor r, Tensor w, Tensor k, Tensor "
      "v, Tensor a, Tensor b, Tensor(a!) y) -> ()");
...
REGISTER_EXTENSION(_v2_5_wkv_fp32_C)
