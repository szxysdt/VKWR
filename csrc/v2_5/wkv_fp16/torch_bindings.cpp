...
static void check_elapsed_t_varlen(torch::Tensor elapsed_t, torch::Tensor slot_indices) {
...
void check_wkv_varlen_inputs(int64_t B, int64_t C, int64_t H,
                              ...
                              torch::Tensor slot_indices, torch::Tensor r,
...
void wkv_seq_fp16_varlen(int64_t B, int64_t total_tokens, int64_t max_t,
  ...
  check_wkv_varlen_inputs(B, C, H, total_tokens, query_start_loc, state,
                           slot_indices, r, w, k, v, a, b);
  check_half_cuda_contig(y, "y");
  TORCH_CHECK(y.size(0) == total_tokens && y.size(1) == C,
              "y must have shape [total_tokens, C]");
  check_elapsed_t_varlen(elapsed_t, slot_indices);

  const int* qsl_ptr = query_start_loc.data_ptr<int>();
  const int* si_ptr = slot_indices.data_ptr<int>();
  wkv_seq_v2_cuda_varlen(static_cast<int>(B), static_cast<int>(max_t), qsl_ptr,
                          si_ptr, static_cast<int>(C), static_cast<int>(H), state,
                          r, w, k, v, a, b, y, elapsed_t);
}
...
void wkv_seq_w0_fp16_varlen(int64_t B, int64_t total_tokens, int64_t max_t,
  ...
  wkv_seq_w0_v2_cuda_varlen(static_cast<int>(B), static_cast<int>(max_t),
                             qsl_ptr, si_ptr, static_cast<int>(C),
...
void advance_i32_varlen(torch::Tensor elapsed, torch::Tensor query_start_loc,
  ...
  TORCH_CHECK(elapsed.dim() == 1, "elapsed must be 1D");
  check_i32_cuda_contig(query_start_loc, "query_start_loc");
  TORCH_CHECK(query_start_loc.dim() == 1,
              "query_start_loc must have shape [B+1]");
  TORCH_CHECK(
       query_start_loc.size(0) == elapsed.size(0) + 1 || (slot_indices.defined() && slot_indices.numel() > 0),
       "query_start_loc must have shape [B+1] or slot_indices must be defined");
  check_i32_cuda_contig(slot_indices, "slot_indices");
  TORCH_CHECK(slot_indices.dim() == 1, "slot_indices must be 1D");
  advance_i32_varlen_cuda(elapsed, query_start_loc, slot_indices);
}

// ===== TORCH_LIBRARY registration (new namespace) =====
TORCH_LIBRARY_FRAGMENT(vkwr_v2_5_wkv, m) {
  m.def(
      "wkv_seq_fp16_varlen(int B, int total_tokens, int max_t, int C, int H, "
      "Tensor query_start_loc, Tensor slot_indices, Tensor(a!) state, Tensor r, "
  ...
  m.def(
      ...
      "H, Tensor query_start_loc, Tensor slot_indices, Tensor(a!) state, Tensor "
...
REGISTER_EXTENSION(_v2_5_wkv_fp16_C)
