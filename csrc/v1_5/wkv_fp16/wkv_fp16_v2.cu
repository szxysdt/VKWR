...
__global__ void __launch_bounds__(CLONE_N, 2) wkv_fp16_v1_clone_kernel(
    const int* __restrict__ query_start_loc, const int C, const int H,
...
void wkv_seq_v2_cuda_impl_varlen(int B, int max_t, const int* query_start_loc,
                                 int C, int H, at::Tensor state, at::Tensor r,
  ...
  if (use_v2_seq(B, max_t)) {
    if (add_w0) {
      wkv_fp16_seq_v2_kernel<true><<<dim3(B * H), dim3(N), 0, stream>>>(
          query_start_loc, C, H, reinterpret_cast<half*>(state.data_ptr()),
...
void wkv_seq_v2_cuda_varlen(int B, int max_t, const int* query_start_loc, int C,
                            int H, at::Tensor state, at::Tensor r, at::Tensor w,
                            at::Tensor k, at::Tensor v, at::Tensor a,
                            at::Tensor b, at::Tensor y, at::Tensor elapsed_t) {
  wkv_seq_v2_cuda_impl_varlen(B, max_t, query_start_loc, C, H, state, r, w,
                              nullptr, false, k, v, a, b, y, elapsed_t);
}

void wkv_seq_w0_v2_cuda_varlen(int B, int max_t, const int* query_start_loc,
                               int C, int H, at::Tensor state, at::Tensor r,
                               at::Tensor w, at::Tensor w0, at::Tensor k,
