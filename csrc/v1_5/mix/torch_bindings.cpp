...
std::vector<torch::Tensor> tmix_mix6_t1_c4096_varlen_fn(
    int64_t B, int64_t total_tokens, torch::Tensor x, torch::Tensor shift_state,
    ...
    torch::Tensor req_id, int64_t threads, int64_t vec, bool half_math) {
  TORCH_CHECK(
...
torch::Tensor cmix_mix_varlen_fn(int64_t B, int64_t total_tokens, int64_t C,
  ...
  return cmix_mix_cuda_varlen(static_cast<int>(B), static_cast<int>(C), x,
                              shift_state, x_k, query_start_loc, req_id);
}

torch::Tensor cmix_sparse_rows_varlen_fn(
    int64_t B, int64_t total_tokens, int64_t C, int64_t F, torch::Tensor x,
    torch::Tensor shift_state, torch::Tensor x_k, torch::Tensor key_fc,
    torch::Tensor value_fc, torch::Tensor query_start_loc,
    torch::Tensor req_id) {
  check_half_cuda_contig(x, "x");
  check_half_cuda_contig(shift_state, "shift_state");
  check_vec(x_k, C, "x_k");
  ...
  return cmix_sparse_rows_cuda_varlen(
      ...
      shift_state, x_k, key_fc, value_fc, query_start_loc, req_id);
}
...
TORCH_LIBRARY_FRAGMENT(vkwr_v1_5_mix, m) {
  m.def(
      "tmix_mix6_varlen(int B, int total_tokens, int C, Tensor x, Tensor(a!) "
      "shift_state, Tensor x_r, Tensor x_w, Tensor x_k, Tensor x_v, Tensor "
      "x_a, Tensor x_g, Tensor query_start_loc, Tensor req_id) -> Tensor[]");
  m.impl("tmix_mix6_varlen", c10::kCUDA, &tmix_mix6_varlen_fn);

  m.def(
      "tmix_mix6_t1_c4096_varlen(int B, int total_tokens, Tensor x, Tensor(a!) "
      "shift_state, Tensor x_r, Tensor x_w, Tensor x_k, Tensor x_v, Tensor "
      "x_a, Tensor x_g, Tensor query_start_loc, Tensor req_id, int threads, "
      "int vec, bool half_math=False) -> Tensor[]");
  ...
  m.def(
      ...
      "shift_state, Tensor x_k, Tensor query_start_loc, Tensor req_id) -> "
      "Tensor");
  m.impl("cmix_mix_varlen", c10::kCUDA, &cmix_mix_varlen_fn);

  m.def(
      "cmix_sparse_rows_varlen(int B, int total_tokens, int C, int F, Tensor "
      "x, Tensor(a!) shift_state, Tensor x_k, Tensor key_fc, Tensor value_fc, "
      "Tensor query_start_loc, Tensor req_id) -> Tensor");
...
REGISTER_EXTENSION(_v1_5_mix_C)
